...
const SystemSettings = () => {
  ...
  const [totalRoomsInput, setTotalRoomsInput] =
    useState("");
  ...
  const [showFloorsList, setShowFloorsList] =
    useState(false);

  const [selectedFloorRooms, setSelectedFloorRooms] =
    useState(null);
  ...
  const [loading2FA, setLoading2FA] =
    useState(false);
  ...
  const isFloorExists = (num) =>
    floors.some(
      (f) =>
        Number(f.floorNumber) === Number(num)
    );
  ...
  const handleCreateRooms = async () => {
    if (
      !totalRoomsInput ||
      Number(totalRoomsInput) <= 0
    ) {
      return toast.error(
        "Enter valid number of rooms"
      );
    }
  ...
  const handleDeleteAllRooms = async () => {
    if (
      !window.confirm(
        "⚠️ Delete ALL rooms permanently?"
      )
    )
      return;
  ...
  const handleCreateFloor = async () => {
    if (!newFloor)
      return toast.error("Enter floor number");
  ...
  const handleEditFloor = async (id) => {
    ...
    if (
      !window.confirm(
        `Change floor to ${editValue}?\nAffects ${floorRooms.length} rooms: ${
          roomList || "none"
        }`
      )
    )
  ...
  const handleBulkAssign = async () => {
    ...
    const getRoomNumber = (roomStr) =>
      Number(roomStr.split("-")[1]);

    const alreadyAssigned = rooms.filter(
      (room) => {
    ...
    if (alreadyAssigned.length > 0) {
      const roomList = alreadyAssigned
        .map((r) => r.roomNumber)
        .join(", ");

      if (
        !window.confirm(
          `Rooms ${roomList} already assigned. Reassign to new floor?`
        )
      ) {
        toast("Cancelled");
        return;
      ...
      <div className="system-settings-page">
        <div className="system-container">
          ...
          <div className="two-column">
            ...
            >
              <div className="card-icon floors">
                🏢
              </div>
        ...
        <div className="system-container">
          ...
          {rooms.length === 0 ? (
            <div className="card">
              ...
              >
                No rooms exist yet. Create your
              ...
              <div className="floor-grid compact-grid">
                {floors.map((floor) => {
                  ...
                  return (
                    <div
                      key={floor._id}
                      className="floor-card compact-card"
                    >
